...
print("Checking for duplicate ayahs across pages or missing ayahs...")
for s in range(1, 115):
    total = hafs_ayat[s-1]
    for a in range(1, total + 1):
        c.execute('SELECT page, line, "left", "right" FROM ayah_highlights WHERE sura = ? AND ayah = ?', (s, a))
        rows = c.fetchall()
        if not rows:
            print(f"MISSING: Sura {s}:{a} has NO highlights anywhere!")
